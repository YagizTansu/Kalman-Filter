...
def correction_step(sensor_data, mu, sigma, landmarks):
    ...
    for i in range(len(ids)):
        ...
        # calculate expected range measurement (FILLed)
        range_exp = np.sqrt( (lx - x)**2 + (ly - y)**2 ) #oklid distance Theorem

        # compute a row of H for each measurement (FILLed)
    ...
    K_help = np.linalg.inv((H @ sigma @ np.transpose(H)) + R)
    K = np.dot(np.dot(sigma,np.transpose(H)),K_help)

    # Kalman correction of mean and covariance (FILLed)
    mu =  mu + np.dot(K,(np.array(Z) - np.array(expected_ranges)))
    sigma = (np.identity(len(sigma)) - (K @ H)) @ sigma
